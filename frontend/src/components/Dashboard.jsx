...
function Dashboard({ projectId, onBack }) {
  ...
  const fetchProjectData = useCallback(async () => {
    ...
    try {
      ...
      // Set project data first (without AI summary)
      const projectDataWithoutSummary = {
        ...data,
        summary: null // Clear the summary to load it separately
      };
      setProjectData(projectDataWithoutSummary);
      
      // Load AI summary separately after GitHub data is loaded
      if (data.repository_stats && Object.keys(data.repository_stats).length > 0) {
        setAiSummaryLoading(true);
        try {
          // Fetch AI summary separately
          const summaryResponse = await makeAuthenticatedRequest(`${backendUrl}/api/v1/projects/${encodeURIComponent(projectId)}/summary`);
          if (summaryResponse.ok) {
            ...
            // Use fallback summary if AI summary fails
            setProjectData(prev => ({
              ...prev,
              summary: data.summary || `Project ${projectId} - GitHub data loaded successfully.`
            }));
          }
        } catch (summaryError) {
          console.error('Error loading AI summary:', summaryError);
